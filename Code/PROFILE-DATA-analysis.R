...
#WITH LOG TRANSFORMATION (log makes it a bit more normal than log1p and we don't have 0s in age anyways)
ggplot(dropout_filtered, aes(sample=log(eta)))+
  stat_qq()+stat_qq_line()+
  facet_wrap(~Dropout)
var.test(log(eta) ~ Dropout, data = dropout_filtered)
t.test(log(eta) ~ Dropout, data = dropout_filtered, na.action = na.omit, var.equal = FALSE)
...
# Step 1: Calculate dropout rate per nation (only nations with > 10 students)
